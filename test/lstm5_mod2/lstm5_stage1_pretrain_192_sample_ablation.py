...
def get_ablation_cfg(ablation_id: str, baseline_ablation: str = "A0") -> dict:
    ...
    if baseline_ablation not in A:
        baseline_ablation = "A0"
    base = dict(A[baseline_ablation])

    if ablation_id in A:
        return dict(A[ablation_id])

    if ablation_id == "B0":
        base.update(disable_branch=False, head_inject_gated=True)
        return base
    if ablation_id == "B1":
        base.update(disable_branch=False, head_inject_gated=False)
        return base
    if ablation_id == "B2":
        base.update(disable_branch=True)
        return base

    if ablation_id == "C0":
        base.update(disable_branch=False, pooling="bilateral_flatten")
        return base
    if ablation_id == "C1":
        base.update(disable_branch=False, pooling="attn")
        return base
