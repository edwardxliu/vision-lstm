...
class ConvPatchMerging(nn.Module):
    ...
    注意：两个都 /2，会变成 /4，不符合你想要的“深层一次 /2”。
    所以这里推荐做：Conv(k=3,s=1) 做混合 + PatchMerging(/2) 做降采样
    """
    def __init__(self, in_dim, out_dim, in_seqlens, norm_bias=True):
        super().__init__()
        self.in_dim = in_dim
        ...
        # 只做混合不降采样
        self.mix = nn.Conv2d(in_dim, in_dim, kernel_size=3, stride=1, padding=1, groups=in_dim, bias=False)
        self.mix_pw = nn.Conv2d(in_dim, in_dim, kernel_size=1, bias=False)

        self.merge = PatchMerging2D(in_dim, out_dim, in_seqlens, norm_bias=norm_bias)
        self.out_seqlens = self.merge.out_seqlens
    ...
    def forward(self, x):
        ...
        x2 = einops.rearrange(x, "b (h w) c -> b c h w", h=H, w=W)
        x2 = self.mix_pw(self.mix(x2))
