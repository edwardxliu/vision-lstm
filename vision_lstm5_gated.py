...
from vision_lstm_util import interpolate_sincos, DropPath
...
class AttnPool(nn.Module):
    """Tiny attention pooling over tokens: (B, N, D) -> (B, D).
    Adds ~D parameters (a single linear scorer).
    """
...
class GatedHeadResidualAdapter(nn.Module):
    """Project branch_vec to head_dim and inject into main_vec with a learned gate.

    - Gate is scalar per-sample by default (very parameter-efficient & stable).
    - Includes LN on main/branch before gating to reduce scale mismatch.
    """
    def __init__(
        self,
        head_dim: int,
        ...
        super().__init__()
        self.proj = nn.Linear(branch_dim, head_dim, bias=True)
        self.alpha = nn.Parameter(torch.ones(head_dim) * init_scale)  # LayerScale-style
        self.ln_main = nn.LayerNorm(head_dim, eps=1e-6)
        self.ln_branch = nn.LayerNorm(head_dim, eps=1e-6)

        self.vector_gate = bool(vector_gate)
        if self.vector_gate:
            self.gate = nn.Linear(head_dim * 2, head_dim, bias=True)
            nn.init.constant_(self.gate.bias, gate_bias)
        else:
            self.gate = nn.Linear(head_dim * 2, 1, bias=True)
            nn.init.constant_(self.gate.bias, gate_bias)

    def forward(self, main_vec: torch.Tensor, branch_vec: torch.Tensor) -> torch.Tensor:
        # main_vec: (B, head_dim), branch_vec: (B, branch_dim)
        b = self.proj(branch_vec)
        m_n = self.ln_main(main_vec)
        b_n = self.ln_branch(b)
        g = torch.sigmoid(self.gate(torch.cat([m_n, b_n], dim=-1)))  # (B,1) or (B,head_dim)
        return main_vec + g * (self.alpha * b)
...
class VisionLSTM2(nn.Module):
    def __init__(self, dim, input_shape, patch_size, depth, output_shape, mode, pooling,
        ...
        # Optional attention pooling (very small param cost)
        self.attn_pool = AttnPool(dim) if (pooling == "attn") else None
        if legacy_norm:
        ...
        if mode == "features":
            ...
            # Gated fusion: learn when/how much to inject branch features
            self.head_adapter = GatedHeadResidualAdapter(head_dim=head_dim, branch_dim=dim, vector_gate=False)
        else:
            raise NotImplementedError
    ...
    def forward(self, x):
        ...
        if self.pooling is None:
            ...
            # Attention Pooling over tokens: [B, N, D] -> [B, D]
            if self.attn_pool is None:
                raise RuntimeError("attn_pool is not initialized; set pooling='attn' in __init__")
            x_main = self.attn_pool(x_main)
            x_main = self.legacy_norm(x_main)
