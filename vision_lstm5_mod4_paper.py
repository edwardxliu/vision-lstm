...
class HeadResidualAdapter(nn.Module):
    ...
    可选门控（推荐，默认开启）：
        delta = proj(branch)
        g = sigmoid( Gate([main, delta]) )   # 标量 gate（B,1），初始偏小更稳
        out = main + g * (alpha ⊙ delta)

    说明：
    - alpha 是 LayerScale 风格的逐通道缩放（初始化很小，避免早期注入过强）
    - gate 让注入强度具备“样本自适应开关”，当分支不稳定/噪声大时更不容易拖后腿
    """
    def __init__(
        self,
        head_dim: int,
        ...
        if self.use_gate:
            ...
            # 让 gate 初始偏小：sigmoid(bias) ~ 0.12 when bias=-2
...
class AttnPool(nn.Module):
    ...
    - 不是 NxN self-attn，只是 1xN 的 cross-attn，开销很小
    - 输出形状: (B, D)
    """
    ...
    def __init__(self, input_channels: int, conv_channels: list,
        ...
        if self.use_dwt:
            ...
            if dwt_fuse == "LL":
                ...
                # safe init: start from identity (z = LL)
...
class DWTPreprocessor(nn.Module):
    ...
    def __init__(self, channels: int, dwt_fuse: str = "LL"):
        ...
        if self.dwt_fuse == "add":
            self.reduce = nn.Conv2d(4 * C, C, kernel_size=1, bias=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.dwt_fuse == "add":
            all4 = torch.cat([ll, lh, hl, hh], dim=1)
            return ll + self.reduce(all4)
        # gated
